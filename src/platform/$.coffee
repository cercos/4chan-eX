...
$.ajax = do ->
  ...
  (url, options={}) ->
    ...
    try
      ...
      $.on r, 'error', -> (c.warn "4chan-eX failed to load: #{url}" unless r.status)
      <% if (type === 'crx') { %>
      # XXX https://bugs.chromium.org/p/chromium/issues/detail?id=920638
      $.on r, 'load', ->
  ...
  $.ajaxPageInit = ->
    $.global ->
      ...
      document.addEventListener '4chanXAjax', (e) ->
        ...
        # connection error or content blocker
        r.onerror = ->
          console.warn "4chan-eX failed to load: #{url}" unless r.status
        if form
          fd = new FormData()
          for entry in form
...
$.crxWorking = ->
  ...
  unless $.crxWarningShown
    msg = $.el 'div',
      `<%= html('4chan-eX seems to have been updated. You will need to <a href="javascript:;">reload</a> the page.') %>`
  ...
  $.getSync = (items, cb) ->
    for key of items when (val2 = $.getValue g.NAMESPACE + key)
      try
        ...
        # XXX https://github.com/ccd0/4chan-eX/issues/2218
