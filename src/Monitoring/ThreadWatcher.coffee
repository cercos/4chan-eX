ThreadWatcher =
  init: ->
    ...
    $.on @markReadButton, 'click', @cb.markAllRead
  ...
  node: ->
    ...
    if data and !data.thumbURL? and @thread.OP?.file?.thumbURL
      $.queueTask =>
        ThreadWatcher.update siteID, boardID, threadID, {thumbURL: @thread.OP.file.thumbURL}
  ...
  cb:
    ...
    markAllRead: ->
      return if $.hasClass @, 'disabled'
      return if !ThreadWatcher.unreadEnabled
      for board in ThreadWatcher.getAll(true, true)
        for {siteID, boardID, threadID, data} in board
          continue unless data and typeof data is 'object'
          continue unless (data.unread or 0) > 0 or (data.quotingYou or 0) > (data.dismiss or 0)
          lastReadPost = data.last or +threadID
          ThreadWatcher.unreaddb.set {siteID, boardID, threadID, val: lastReadPost}
    ...
    dragStart: (e) ->
      return if e.target.closest?('.fa-times, .watcher-mark-read')
      ThreadWatcher.draggingLine = @
      $.addClass @, 'dragging'
      if e.dataTransfer
        e.dataTransfer.effectAllowed = 'move'
        e.dataTransfer.setData 'text/plain', ThreadWatcher.keyFromLine @
    dragOver: (e) ->
      return unless ThreadWatcher.draggingLine and ThreadWatcher.draggingLine isnt @
      e.preventDefault()
      {before} = ThreadWatcher.dropPosition @, e
      $.addClass @, 'drag-over'
      @dataset.dropBefore = !!before
    dragLeave: ->
      $.rmClass @, 'drag-over'
      delete @dataset.dropBefore
    drop: (e) ->
      return unless ThreadWatcher.draggingLine and ThreadWatcher.draggingLine isnt @
      e.preventDefault()
      source = ThreadWatcher.draggingLine
      target = @
      {before} = ThreadWatcher.dropPosition target, e
  ...
  fetchBoard: (board, deep) ->
    ...
    for thread in board
      ...
      if !data.isDead and data.last isnt -1
        ...
        force = thread.force = true if ThreadWatcher.showThumbnails() and !data.thumbURL?
  ...
  parseBoard: (board, url) ->
    ...
    for thread in board
      ...
      if threads[threadID]
        ...
        if ThreadWatcher.showThumbnails() and !data.thumbURL?
          ThreadWatcher.fetchStatus thread
  ...
  parseStatus: (thread, isArchiveURL) ->
    ...
    if @status is 200 and @response
      ...
      thumbURL = ThreadWatcher.getOPThumbURL {siteID, boardID, postObj: @response.posts[0]}
      if isDead and Conf['Auto Prune']
        ThreadWatcher.rm siteID, boardID, threadID
        return

      return if last is data.last and isDead is data.isDead and isArchived is data.isArchived and (!thumbURL or thumbURL is data.thumbURL)
      ...
      newData or= {}
      if thumbURL? and thumbURL isnt data.thumbURL
  ...
  getOPThumbURL: ({siteID, boardID, thread, postObj}) ->
    return thread.OP.file.thumbURL if thread?.OP?.file?.thumbURL
    return unless postObj
    site = g.sites[siteID]
    if site?.Build?.parseJSON
      try
        post = site.Build.parseJSON postObj, {siteID, boardID}
        return post.file.thumbURL if post?.file?.thumbURL
      catch
        null
    if postObj.tim? and site?.urls?.thumb
      return site.urls.thumb({siteID, boardID}, "#{postObj.tim}s.jpg")
    return

  getAll: (groupByBoard, ignoreCurrentBoard) ->
    all = []
    for siteID, boards of ThreadWatcher.db.data
      for boardID, threads of boards.boards
        if Conf['Current Board'] and !ignoreCurrentBoard and (siteID isnt g.SITE.ID or boardID isnt g.BOARD.ID)
          continue
        if groupByBoard
          all.push (cont = [])
  ...
  sortByOrder: (a, b) ->
    aOrder = if typeof a.data.order is 'number' then a.data.order else Number.POSITIVE_INFINITY
    bOrder = if typeof b.data.order is 'number' then b.data.order else Number.POSITIVE_INFINITY
    aOrder - bOrder or
    (if a.siteID < b.siteID then -1 else if a.siteID > b.siteID then 1 else 0) or
    (if a.boardID < b.boardID then -1 else if a.boardID > b.boardID then 1 else 0) or
  ...
  keyFromLine: (line) ->
    "#{line.dataset.siteID}/#{line.dataset.fullID}"
  ...
  clearDragState: ->
    ...
    for line in $$ '#watched-threads > div', ThreadWatcher.dialog
      $.rmClass line, 'drag-over'
      delete line.dataset.dropBefore

  reorderByKeys: (sourceKey, targetKey, before) ->
    return if sourceKey is targetKey
    threads = ThreadWatcher.getAll(false, true)
    sourceIndex = -1
    targetIndex = -1
    for thread, i in threads
      key = ThreadWatcher.threadKey thread
      sourceIndex = i if key is sourceKey
      targetIndex = i if key is targetKey
    return if sourceIndex < 0 or targetIndex < 0
    [source] = threads.splice sourceIndex, 1
    targetIndex = -1
    for thread, i in threads
      if ThreadWatcher.threadKey(thread) is targetKey
        targetIndex = i
        break
    return if targetIndex < 0
    insertAt = if before then targetIndex else targetIndex + 1
  ...
  persistOrder: (threads) ->
    for thread, i in threads
      order = i + 1
      continue if thread.data.order is order
  ...
  makeLine: (siteID, boardID, threadID, data) ->
    ...
    markRead.classList.toggle 'disabled', !ThreadWatcher.unreadEnabled or !((data.unread or 0) > 0 or (data.quotingYou or 0) > (data.dismiss or 0))
  ...
  build: ->
    ...
    for {siteID, boardID, threadID, data} in threads
      ...
      # Add missing thumbnail URL for already watched threads.
      if not data.thumbURL? and siteID is g.SITE.ID and (thread = g.threads.get("#{boardID}.#{threadID}")) and thread.OP?.file?.thumbURL
  ...
  refreshIcon: ->
    ...
    ThreadWatcher.markReadButton.classList.toggle 'disabled', !ThreadWatcher.unreadEnabled or !hasUnread
    ThreadWatcher.markReadButton.title = if !ThreadWatcher.unreadEnabled
  ...
  add: (thread, cb, manual) ->
    ...
    data.thumbURL = thread.OP.file.thumbURL if thread.OP?.file?.thumbURL
  ...
  addRaw: (boardID, threadID, data, cb, manual) ->
    ...
    else if ThreadWatcher.trackRepliesState() or ThreadWatcher.showThumbnails()
      ThreadWatcher.fetchStatus thread
  ...
  menu:
    ...
    addMenuEntries: ->
      ...
      entries.push
        ...
        open: ->
          disabled = !ThreadWatcher.unreadEnabled or !$('.replies-unread, .replies-quoting-you', ThreadWatcher.list)
