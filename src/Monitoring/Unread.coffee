Unread =
  ...
  update: ->
    ...
    if Conf['Unread Count']
      countStyle = Conf['Unread Title Count']
      titleQuotingYou = if countStyle in ['quoted', 'quoted-hide-zero'] and countQuotingYou then '(!) ' else ''
      titleCount = if count or countStyle not in ['hide-zero', 'quoted-hide-zero'] then "(#{count}) " else ''
