ThreadStats =
  ...
  init: ->
    ...
    if Conf['Detailed Thread Stats']
      @[if g.SITE.isPrunedByAge?(g.BOARD) then 'showPurgePos' else 'showPage'] = true

    statsHTML = `<%= html(
      '<span id="post-count">?</span> / <span id="file-count">?</span>' +
      '?{Conf["Detailed Thread Stats"] && g.SITE.hasIPCount}{ / <span id="ip-count">?</span>}' +
      '?{Conf["Detailed Thread Stats"]}{ / <span id="page-count">?</span>}'
    ) %>`
    statsTitle = 'Posts / Files'
    statsTitle += ' / IPs'  if Conf['Detailed Thread Stats'] and g.SITE.hasIPCount
    statsTitle += (if @showPurgePos then ' / Purge Position' else ' / Page') if Conf['Detailed Thread Stats']
