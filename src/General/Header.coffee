Header =
  init: ->
    ...
    Main.ready ->
      ...
      if (Header.bottomBoardList = $ g.SITE.selectors.boardListBottom)
        currentBoardID = g.BOARD?.ID
        for a in $$ 'a', Header.bottomBoardList
          a.className = 'current' if currentBoardID and a.hostname is location.hostname and a.pathname.split('/')[1] is currentBoardID
  ...
  mapCustomNavigation: (t) ->
    ...
    if boardID is 'current'
      return $.tn text or '' unless currentBoardID
      if location.hostname in ['boards.4chan.org', 'boards.4channel.org']
        boardID = currentBoardID
      else
        a = $.el 'a',
          href: "/#{currentBoardID}/"
          textContent: text or decodeURIComponent(currentBoardID)
          className: 'current'
        if /-nt/.test t
          a.target = '_blank'
          a.rel = 'noopener';
    ...
    a = do ->
      ...
      if g.VIEW in ['catalog', 'archive'] and (urlV = Get.url g.VIEW, {siteID: '4chan.org', boardID})
        a.href = urlV
      a.className = 'current' if a.hostname is location.hostname and boardID is currentBoardID
      a

    a.textContent = if /-title/.test(t) or /-replace/.test(t) and a.hostname is location.hostname and boardID is currentBoardID
  ...
  enableDesktopNotifications: ->
    ...
    el = $.el 'span',
      `<%= html(
        ...
        '[<a href="' + meta.faq + '#why-is-4chan-eX-asking-for-permission-to-show-desktop-notifications" target="_blank">FAQ</a>]<br>' +
