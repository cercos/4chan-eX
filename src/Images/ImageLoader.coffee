ImageLoader =
  init: ->
    ...
    replace = Conf['Replace Thumbnails']
  ...
  node: ->
    return if @isClone
    for file in @files
      ImageLoader.replaceVideo @, file if Conf['Replace Thumbnails'] and file.isVideo
  ...
  prefetch: (post, file) ->
    ...
    replace = Conf['Replace Thumbnails'] and !/spoiler/.test(thumb.src or thumb.dataset.src)
