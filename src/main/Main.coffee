Main =
  init: ->
    ...
    # Detect multiple copies of 4chan-eX
    ...
    $.on d, '4chanXInitFinished', ->
      if Main.expectInitFinished
        ...
        new Notice 'error', 'Error: Multiple copies of 4chan-eX are enabled.'
    ...
    Conf['Replace GIF'] = false
    Conf['Replace JPG'] = false
    Conf['Replace PNG'] = false
    Conf['Replace WEBM'] = false
    Conf['IP Count in Stats'] = true
    Conf['Page Count in Stats'] = true
    Conf['Quoted Title'] = false
    Conf['Hide Unread Count at (0)'] = false
    Conf['Remove Thread Excerpt'] = false
    ...
    ($.getSync or $.get) items, (items) ->
      ...
      $.asap docSet, ->
        ...
        setAlias = (key, value) ->
          return if items[key]?
          items[key] = aliasChanges[key] = value
        setAlias 'Replace Thumbnails', !!(items['Replace GIF'] or items['Replace JPG'] or items['Replace PNG'] or items['Replace WEBM'])
        setAlias 'Detailed Thread Stats', !!(items['IP Count in Stats'] ? Conf['IP Count in Stats']) or !!(items['Page Count in Stats'] ? Conf['Page Count in Stats'])
        unless items['Thread Title']?
          items['Thread Title'] = aliasChanges['Thread Title'] = if items['Remove Thread Excerpt'] then 'board' else 'excerpt'
        unless items['Unread Title Count']?
          quoted = !!items['Quoted Title']
          hideZero = !!items['Hide Unread Count at (0)']
          items['Unread Title Count'] = aliasChanges['Unread Title Count'] = if quoted and hideZero
            'quoted-hide-zero'
          else if quoted
            'quoted'
          else if hideZero
            ...
            'always'
        $.set aliasChanges if Object.keys(aliasChanges).length
  ...
  initFeatures: ->
    ...
    for [name, feature] in Main.features
      continue if isBoardlessPage and name isnt 'Custom CSS'
  ...
  setClass: ->
    ...
    setStyle = ->
      ...
      $.rm div
      rgb = bgColor.match(/[\d.]+/g) or ['0', '0', '0', '1']
  ...
  handleErrors: (errors) ->
    # Detect conflicts with 4chan-eX v2
